...
class image_folder_publisher:
    def __init__(self):
        ...
        self._caminfo_topic = '/navigation/left_camera/camera_info'
        ...
        self._camerainfo_publisher = rospy.Publisher(self._caminfo_topic, CameraInfo, queue_size=1)
                    ...
                    if not rospy.is_shutdown():
                        if isfile(join(self._image_folder, f)):
                            ...
                            if cv_image is not None:
                                ...
                                # cam_info pub
                                caminfo = CameraInfo()
                                caminfo.header.frame_id = self._frame_id
                                caminfo.header.stamp = ros_msg.header.stamp
                                caminfo.height = 720
                                caminfo.width = 1280
                                # caminfo.distortion_model = "plumb_bob"
                                caminfo.D = [0, 0, 0, 0, 0]
                                caminfo.K = [261.77886962890625, 0, 319.94610595703125,
                                             0, 261.77886962890625, 179.56675720214844,
                                             0, 0, 1]
                                caminfo.R = [1, 0, 0,
                                             0, 1, 0,
                                             0, 0, 1]
                                caminfo.P = [261.77886962890625, 0, 319.94610595703125,
                                             0, 0, 261.77886962890625,
                                             179.56675720214844, 0, 0,
                                             0, 1, 0]

                                self._camerainfo_publisher.publish(caminfo)
                                rospy.loginfo("[%s] Published %s", self.__app_name, join(self._image_folder, f))
                            else:
                                rospy.loginfo("[%s] Invalid image file %s", self.__app_name,
                                              join(self._image_folder, f))
                            ros_rate.sleep()
                    else:
                        return
